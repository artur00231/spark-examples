package Railway with SPARK_Mode is
   ...
   function Correct_E return Boolean
   is
     (
        (if Segment_State.Left = Occupied_Moving_Right then
              Signal_State.Left_Middle = Green) and
        (if Segment_State.Middle = Occupied_Moving_Right then
              Signal_State.Middle_Right = Green) and
        (if Segment_State.Right = Occupied_Moving_Left then
              Signal_State.Right_Middle = Green) and
        (if Segment_State.Middle = Occupied_Moving_Left then
   ...
   procedure Open_Route (Route: in Route_Type; Success: out Boolean)
     with
       ...
       Pre => Correct_Signals and Correct_Segments and Correct_E,
     Post => Correct_Signals and Correct_Segments and Correct_E and
     (if Success = True then Segment_State'Old /= Segment_State) and
     (if Success = False then Segment_State'Old = Segment_State and Signal_State'Old = Signal_State);
